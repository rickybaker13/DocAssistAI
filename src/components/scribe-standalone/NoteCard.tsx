...
export const NoteCard: React.FC<Props> = ({ note, onDelete }) => {
  ...
  const borderAccent = note.status === 'finalized'
    ? 'border-l-emerald-400'
    : 'border-l-teal-400';

  return (
    <div className={`bg-slate-800 border border-slate-700 border-l-4 ${borderAccent} rounded-xl p-4 flex items-start justify-between gap-3 hover:bg-slate-700 hover:border-slate-600 transition-all duration-150`}>
      ...
      <div className="flex-shrink-0">
        {!confirmDelete ? (
          <button
            ...
            className="text-slate-600 hover:text-red-400 hover:bg-slate-600 p-1.5 rounded-lg transition-all duration-150"
          >
            <Trash2 size={16} />
          </button>
        ) : (
